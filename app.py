...
class BackgroundGUI:
    ...
    def login_dashboard(self):
        ...
        self.search_entry.place(x=600, y=10) 
        ...
        self.fotoB = tk.PhotoImage(file="infokecil.png")
        image_path10= 'tombol pg.png'
...
#--------------------------------dashboard mauk /daftar ----------------------#
    ...
    def fade_in_background(self, alpha=0):
        if hasattr(self, 'canvas') and hasattr(self, 'window') and self.canvas.winfo_exists():
            if alpha < 1:
            ...
            print("Canvas or window does not exist")
...
#____________________________________________PASSING GRADE_________________________________#
    def passing_grade(self):
        ...
        self.current_menu = tk.Frame(self.window) 
        ...
        for col in columns:
            ...
            self.tree.column(col, minwidth=0, width=165, anchor='center')
    ...
    # sorting kecil ke besar(passing grade)
    ...
    # soerting_besar ke kecil(passing grade)
        ...
        self.search_entry = customtkinter.CTkEntry(self.current_menu, width=163, corner_radius=15, border_color="#FFF9D0", bg_color="#7fbcd0")
    ...
    def pecet_listbox_jurusan(self, event):
        ...
        if selection:
            ...
            sentinel = {"Fakultas": selected_fakultas}
            filtered_data = []
...
#__________________________________________INFONESA_____________________________#
    ...
    def infofmipa(self):
        ...
        self.dropdown.config(font=('consolas', 30), width=20)  
...
# ____________________________________________TRY-U____________________________________________________________________
    ...
    def formulir(self):
        ...
        self.minat.trace('w', self.update_kategori) 
        ...
        self.jurusan_entry.bind("<KeyRelease>", self.update_search1)  
        ...
        self.jurusan_entry2.bind("<KeyRelease>", self.update_search2)  
    ...
    def read_csvvv(self):
        ...
        with open("datapg.csv", 'r', encoding='utf-8-sig') as file:
            ...
            next(csv_reader)  
            for row in csv_reader:
                self.data_jurusan.append(row[1])  
    ...
    def update_search(self, id):
        keyword = ""
        if id == 1: 
            keyword = self.jurusan.get().strip()  
        elif id == 2: 
            keyword = self.jurusan2.get().strip()  
    ...
    def pencet_listbox_jurusan(self, event):
        if self.resulttt.curselection():
            ...
            self.jurusan_entry.insert(0, selected_text)  
            self.resulttt.place_forget()  
    ...
    def pencet_listbox_jurusan2(self, event):
        if self.resulttt2.curselection():
            ...
            self.jurusan_entry2.delete(0, tk.END)  
            self.jurusan_entry2.insert(0, selected_text)  
            self.resulttt2.place_forget() 
    ...
    def check_and_delete_data(self):
        ...
        if self.check_username(username):
            ...
            self.lbe_selesai = False
            self.pm_selesai = False      
            messagebox.showinfo("Success", "selamat mengerjakan kembali!")
        else:
    ...
    def submit_and_save(self):
        ...
        messagebox.showinfo("Success", "Silahkan mengerjakan!")
...
#
    ...
    def end_quiz(self):
        if messagebox.askyesno("Confirmation", "Apakah anda ingin mengakhiri pengerjaan subtest ini?"):
            self.hitung_skor()
            self.layarsoal.destroy() 
            self.tryU()  
            if self.pu_selesai and self.ppu_selesai and self.pk_selesai and self.pbm_selesai and self.pm_selesai and self.lbe_selesai and self.lbi_selesai:
                self.save_results(self.list_skor)  
                print(self.list_skor)  
    ...
    def save_results(self, kumpulan_skor):
        ...
        try:
            ...
            if row_updated:
                ...
                print("Data user tidak ditemukan atau sudah diperbarui sebelumnya.")
        ...
        with open("hasil_kuis.csv", "r", newline='') as file:
            ...
            for row in reader:
                ...
                for field in fieldnames[5:12]:
                    if row[field] == '-' or not row[field].isdigit():
    ...
    def rekomakhir(self):
        ...
        with open('hasil_kuis.csv', mode='r', newline='') as file:
            reader = csv.DictReader(file)
            for i in reader:
                if str(i["user"]) == str(self.username.get()): 
        ...
        if c == 1 and user_score is not None:
            a = self.display_data(self.username.get()) 
            ...
            with open('datapg.csv', mode='r', newline='') as file:
                ...
                if hasattr(self, 'recommended_programs_str') and self.recommended_programs_str:
                    ...
                    tk.Label(image_label, wraplength= 400,text=f"{self.recommended_programs_str}", bg='white', font=("Arial", 15), fg="black").place(x=22, y=394)
        else:
        ...
        close_button.place(x=150, y=444)
